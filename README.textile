...
(TODO: ユーザ用と plugcond-templates リポジトリからインストールされた plugconf は分けた方がいい？)
...
* `/templates/{site}/{user}/{name}.{json or/and vim}`
...
* volt profile init {profile name}
** {profile name} を持った profile を作成

* volt profile add {profile name} {repository}
** {profile name} の profile に {repository} のプラグインを追加

* volt profile rm {profile name} {repository}
** {profile name} の profile から {repository} のプラグインを削除
